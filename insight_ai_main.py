...
- Tailor your reasoning to the document type.
...
- Prefer the most directly supported answer.
- Do not repeat the same evidence twice.
- Mention source file names only when useful.
...
Requirements:
- Prioritise repeated themes across sources, not just one source.
...
def dedupe_documents(documents):
    ...
    for doc in documents:
        ...
        normalized = normalize_for_dedupe(doc.page_content)[:900]
...
def extract_query_terms(question: str) -> Set[str]:
    return set(tokenize_text(question))


def score_doc_for_question(doc, query_terms: Set[str]) -> int:
...
def select_relevant_results(results, question: str, final_k: int = QA_FINAL_K):
    query_terms = extract_query_terms(question)
    scored = []
    for idx, doc in enumerate(results):
        overlap = score_doc_for_question(doc, query_terms)
    ...
    for overlap, _, doc in scored:
        source = source_label(doc)
        snippet_key = normalize_for_dedupe(doc.page_content)[:220]
        if snippet_key in seen_snippets:
            continue
        if source not in seen_sources or len(selected) < max(2, final_k - 1):
            selected.append(doc)
...
def build_doc_type_hint(document_type: str) -> str:
    hints = {
        "Policy / Framework": "Focus on governance, controls, compliance obligations, and strategic implementation gaps.",
        "Meeting Notes": "Focus on action items, blockers, ownership gaps, decisions, and operational follow-up.",
        "Budget / Financial": "Focus on budget pressure, cost overruns, funding gaps, and financial controls.",
        "Security / Technical": "Focus on vulnerabilities, access control, control deficiencies, privacy exposure, and technical safeguards.",
        "Internal Memo / Review": "Focus on recurring business issues, operational weaknesses, and management actions.",
    }
    return hints.get(document_type, "Focus on the most important business risks, evidence, and management actions.")


def build_doc_specific_risk_prompt(document_text: str, document_type: str) -> str:
    return RISK_PROMPT_TEMPLATE.format(document_text=document_text, document_type=f"{document_type}. {build_doc_type_hint(document_type)}")


def build_balanced_collection_text(documents, per_file_limit: int = 1800, max_files: int = 6) -> str:
    grouped = defaultdict(list)
    for doc in documents:
        grouped[source_label(doc)].append(doc.page_content)

    blocks = []
    for source in sorted(grouped)[:max_files]:
        file_text = "\n\n".join(t for t in grouped[source] if t)
        blocks.append(f"Source: {source}\nContent: {file_text[:per_file_limit]}")
    return "\n\n".join(blocks)[:9000]
...
def build_vectorstore(docs_dir, excluded_file_names_tuple, excluded_folder_names_tuple, excluded_keywords_tuple):
    ...
    chunks = dedupe_documents(splitter.split_documents(documents))
...
def build_context(results) -> str:
    ...
    for doc in results:
        context_blocks.append(
            f"Source: {source_label(doc)}\nEvidence: {short_snippet(doc.page_content, 420)}"
        )
    return "\n\n".join(context_blocks)
...
def parse_qa_answer(raw_answer: str, results) -> Tuple[str, List[str]]:
    ...
    if evidence_match:
        parsed = [line.strip(" -") for line in evidence_match.group(1).splitlines() if line.strip()]
        seen = set()
        for line in parsed:
    ...
    if not evidence_lines:
        seen = set()
        for doc in results:
            line = f"{source_label(doc)}: {short_snippet(doc.page_content, 120)}"
            norm = normalize_for_dedupe(line)
            if norm in seen:
                continue
            seen.add(norm)
...
def ask_question(vectorstore, question):
    ...
    initial_results = vectorstore.similarity_search(question, k=QA_INITIAL_K)
    results = select_relevant_results(initial_results, question, final_k=QA_FINAL_K)
...
def summarise_all_documents(documents):
    llm = OllamaLLM(model=CHAT_MODEL)
    combined_text = build_balanced_collection_text(documents)
    prompt = f"""
You are an offline AI assistant for confidential documents.
...
def generate_collection_risk_report(documents):
    ...
    for doc in documents:
        try:
            doc_types.append(classify_document_type(doc.page_content, source_label(doc)))
        except Exception:
            continue

    if doc_types:
        doc_type_counts = Counter(doc_types)
        dominant_type, dominant_count = doc_type_counts.most_common(1)[0]
        doc_type = dominant_type if dominant_count >= 2 else "Mixed Collection"
    else:
        doc_type = classify_document_type(combined_text, "collection") or "Mixed Collection"

    llm_output = llm.invoke(build_doc_specific_risk_prompt(combined_text, doc_type))
...
def generate_file_risk_report(documents, selected_file):
    ...
    llm_output = llm.invoke(build_doc_specific_risk_prompt(file_text, doc_type))
...
def generate_executive_summary(documents):
    llm = OllamaLLM(model=CHAT_MODEL)
    combined_text = build_balanced_collection_text(documents, per_file_limit=1400, max_files=8)
...
def main():
    st.set_page_config(page_title="INSIGHT.AI Prototype V11 Lite", layout="wide")
    init_state()

    st.title("INSIGHT.AI Prototype V11 Lite")
