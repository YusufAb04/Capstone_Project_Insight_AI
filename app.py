...
def render_file_explorer() -> None:
    ...
    with tabs[0]:
        ...
        is_local = file_path and not file_path.startswith("uploaded://")

        delete_disk = False
        if is_local:
            delete_disk = st.checkbox(
                "Also delete the file from disk (permanent — cannot be undone)",
                key=f"del_disk_{file_choice}",
            )

        confirm = st.checkbox(
            f"I confirm I want to remove **{file_choice}** from the index",
            key=f"del_confirm_{file_choice}",
        )

        if st.button("Delete", type="secondary", key=f"del_btn_{file_choice}"):
            if not confirm:
                st.warning("Tick the confirmation checkbox first.")
            else:
                # Remove from SQLite (CASCADE removes analysis_results too)
                try:
                    conn = get_connection(st.session_state.db_path)
                    conn.cursor().execute("DELETE FROM files WHERE file_path = ?", (file_path,))
                    conn.commit()
                    conn.close()
                except Exception as exc:
                ...
                if _CHROMA_AVAILABLE and _RAG_IMPORTS_OK:
                    try:
                        from src.rag.vector_store import delete_by_file_path as _chroma_del
                        deleted_chunks = _chroma_del(file_path, persist_dir=_chroma_dir())
                    except Exception as chroma_exc:
                        st.warning(f"Removed from index, but ChromaDB cleanup failed: {chroma_exc}. Use Operations → AI/RAG Settings → Reset Collection to fully clean up.")

                # Optionally delete from disk
                if delete_disk and is_local:
                    try:
                ...
                log_audit(
                    "delete_file",
                    file_choice,
                    f"Deleted from index{' and disk' if delete_disk else ''}",
                )
                st.success(f"**{file_choice}** has been removed.")
                st.rerun()
